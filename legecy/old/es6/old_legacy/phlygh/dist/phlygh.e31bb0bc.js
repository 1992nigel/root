...
var likeWhoListings = function likeWhoListings(child, id) {
  ...
  var already_liked = false; // see if already liked
  ...
  if (already_liked == false) {
    ...
    console.log(id); // update users data

    var changeThisUsersValue;
    var user_Ref = gnougnRef.child('roster');
    ...
    changeThisUsersValue.liked_posts.push({
      id: id,
      time: "".concat(state.data.time)
    });
    ...
    ;
    user_Ref.child(state.data.user.id).set(changeThisUsersValue); // Set firebase data
...
var dislikeWhoListings = function dislikeWhoListings(child, id) {
  ...
  if (already_liked == true) {
    ...
    for (var i = 0; i < state.data.user.liked_posts.length; i++) {
      ...
      if (state.data.user.liked_posts[i].id != changeThisValue.id) {
        old_liked_list.push({
          id: state.data.user.liked_posts[i].id,
          time: "".concat(state.data.time)
        });
      }
    ...
    setTimeout(function () {
      if (fb_response != null) {
        ...
        if (user_logged == true) {
          ...
          ;
          var tag_picked = user_algorithm[Math.floor(Math.random() * user_algorithm.length)].name; //alert(tag_picked.name);

          for (var i = 0; i < fb_response.gnougn.library.length; i++) {
            var matched = false;

            for (var tags_i = 0; tags_i < fb_response.gnougn.library[i].tags.length; tags_i++) {
              var particular_tag = fb_response.gnougn.library[i].tags[tags_i].name; //alert(tag_picked);
              ...
              if (matched != true) {//alert(fb_response.gnougn.library[i]);
...
var Handle_Repeat_Firebase_Validation = setInterval(function () {
  ...
  if (fb_writeable == true) {
    ...
    if (fb_response.gnougn.library != null) {
      ...
      fb_response.gnougn.skills = result_skills; // now that data has been received
  ...
  setTimeout(function () {
    ...
    if (state.app.route == 'route_home') {
      ...
      setTimeout(function () {
        ...
        // desktop
        if (!/Android|webOS|iPhone|iPad|iPod|BlackBerry/i.test(navigator.userAgent)) {
          document.getElementById('root').innerHTML = "".concat(element_bgs);
        }
      ...
      setTimeout(function () {
        ...
        ;
        Handle_Firebase_Refs_and_Render(); // add event listeners
...
var Handle_Dice = function Handle_Dice() {
  return Math.floor(Math.random() * 6) + 1;
};

var Handle_Random = function Handle_Random() {
...
window.onload = function () {
  ...
  Handle_Firebase_Refs_and_Render();
  Handle_Repeat_Firebase_Validation; //check_local_user();
};
